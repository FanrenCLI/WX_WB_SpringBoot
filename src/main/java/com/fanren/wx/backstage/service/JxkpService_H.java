...
@Service
public class JxkpService_H {
    @Autowired
    MongoTemplate mongoTemplate;

    public String jxkpList(){
        List<String> data = mongoTemplate.findAll(String.class,"jxkp");
        String result = "{\"jxkp\":"+data+"}";
        return result;
    }

    public String Getjxkp(String name){
        Query query = new Query(Criteria.where("title").is(name));
        String result = "{\"jxkp\":"+mongoTemplate.find(query,String.class,"jxkp").get(0)+"}";
        return result;
    }

    public long jxkp_number(){
        return mongoTemplate.count(new Query(),"jxkp");
    }

    public String jxkp_upload(MultipartFile file) throws Exception {
        //判断上传的文件是否存在
        Query query = new Query(Criteria.where("title").is(file.getOriginalFilename().split("\\.")[0]));
        List<String> list = mongoTemplate.find(query,String.class,"jxkp");
        if (list.size()==1){
            return "存在";
        }
        //上传文件
        ...
        //获取教务通知总量
        long count = mongoTemplate.count(new Query(), "jxkp");
        count++;
        ...
        mongoTemplate.insert(jsonObject,"jxkp");
        return "成功";
    }

    public void jxkp_download(String name, HttpServletResponse response) throws IOException {
        Query query = new Query(Criteria.where("title").is(name));
        String data = mongoTemplate.find(query,String.class,"jxkp").get(0);
        JSONObject jxkp = JSONObject.parseObject(data);
        String filePath = jxkp.getString("src");
    ...
    public void jxkp_delete(String name) throws IOException {
        //删除文件
        Query query = new Query(Criteria.where("title").is(name));
        String delete_data = mongoTemplate.find(query,String.class,"jxkp").get(0);
        JSONObject jxkp = JSONObject.parseObject(delete_data);
        FtpUtils ftpUtils = new FtpUtils();
        String filePath = jxkp.getString("src");
        String fileName =filePath.substring(filePath.lastIndexOf("/")+1) ;
        ftpUtils.deleteFile(fileName);
        //删除mongodb中数据
        mongoTemplate.remove(query,"jxkp");
        //获取删除数据的count
        double delete_data_count =Double.parseDouble( jxkp.getString("count"));
        //获取所有数据数量
        long sum = mongoTemplate.count(new Query(),"jxkp");
        ...
        if (delete_data_count!=sum){
            for (double i=delete_data_count+1;i<=sum+1;i++){
                ...
                String data = mongoTemplate.find(query1,String.class,"jxkp").get(0);
                ...
                mongoTemplate.updateMulti(query1,update,"jxkp");
            }
        }
